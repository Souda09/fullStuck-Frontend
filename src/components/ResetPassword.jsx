...
const ResetPassword = () => {
  const { token } = useParams(); // URL parameters se resetToken fetch karna
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Axios POST call to backend reset-password API with token
      const response = await API.post(`/reset-password/${token}`, { password });
      setSuccess(response.data.message || 'Password reset successful!');
      
      console.log('🔒 [AUTH SYSTEM] Password Reset Successful. Redirecting to Login...');

      setTimeout(() => {
        navigate('/login');
      }, 2500);
    } catch (err) {
      const errMsg = err.response?.data?.message || err.message || 'Link is invalid or has expired';
      setError(errMsg);
    } finally {
      setLoading(false);
    }
