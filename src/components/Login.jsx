      ...
      <div className="bg-[var(--bg-secondary)] p-8 rounded-2xl shadow-2xl w-full max-w-md border border-[var(--border-color)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="password" placeholder="Password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} required
            className="w-full px-4 py-3 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-lg text-[var(--text-primary)] focus:ring-2 focus:ring-[var(--accent)] outline-none" />
          <button type="submit" className="w-full bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white font-semibold py-3 rounded-lg transition">Login</button>
        </form>
        <p className="text-center mt-4 text-[var(--text-secondary)] text-sm">Don't have an account? <Link to="/signup" className="text-[var(--accent)] hover:underline">Sign up</Link></p>
      </div>
    </div>
  );
};
